...
Build-time tool that reads the declarative manifest for document conversions and
emits concrete Converter classes as Python files under `colossal/converters/document/`.
"""
...
MANIFEST_PATH = Path(__file__).parents[1] / "resources" / "formats" / "document.json"
OUTPUT_DIR = Path(__file__).parents[1] / "converters" / "document"
...
class {class_name}(BaseConverter):
    """Convert document to {to_fmt} using configured engine.
    ...
    id = 'document-to-{to_fmt}'
    name = 'Document to {to_fmt_upper}'
    ...
    def factory(task: ConversionTask, report_progress=None):
        ...
        # preset options injected at build time
...
def _build_mode_block(conv: dict, engine: str) -> str:
    ...
    if mode == 'render':
        # render using office or pandoc depending on engine key
        lines.extend([
            f"        # Render using {engine}",
            f"        cmd = [shutil.which({engine_literal}), '--headless', '--convert-to', '{'{'}to_fmt{'}'}', '--outdir', str(dst.parent), str(src)]",
        ])

    elif mode == 'transcode':
        # office-to-office transcode
        lines.extend([
            f"        cmd = [shutil.which({engine_literal}), '--headless', '--convert-to', '{'{'}to_fmt{'}'}', '--outdir', str(dst.parent), str(src)]",
        ])

    elif mode == 'extract':
        # extract text/content from pdf using pdf toolchain
        lines.extend([
...
def main(argv=None):
    p = argparse.ArgumentParser(prog='document_converter_builder')
    p.add_argument('--manifest', '-m', type=Path,
                   default=MANIFEST_PATH,
                   help='Path to document manifest JSON')
